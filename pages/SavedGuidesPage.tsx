...
export const SavedGuidesPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const profileIds = userProfileService.getFavorites().instructors;
        setSavedIds(profileIds);
        
        const allGuides = await dbService.getGuides();
        setGuides(allGuides);
      } finally {
        setLoading(false);
      }
  ...
  const filteredGuides = savedIds
    .slice()
    .reverse() // Newest first
    .map(id => guides.find(g => g.id === id))
    .filter((g): g is Instructor => {
      if (!g) return false;
    ...
    <div className="yt-page">
      <div style={{ padding: '24px', backgroundColor: 'var(--bg-main)', borderBottom: '1px solid var(--border)' }}>
        ...
        <h1 className="yt-h1" style={{ fontSize: '1.8rem', marginBottom: 4 }}>
          {tsafe('saved.guidesTitle', 'Saved Guides')}
        </h1>
      </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24, paddingTop: 16 }}>
            {filteredGuides.map(guide => (
              <div key={guide.id}>
                <InstructorCard instructor={guide} showMatchBadge={false} />
              </div>
            ))}
          </div>
        ) : (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-2)' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 500, marginBottom: 12 }}>{tsafe('saved.noGuides', 'No saved guides yet')}</h3>
            <button 
